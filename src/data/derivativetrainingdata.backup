...
public:
	...
	//typedef	boost::shared_ptr<DerivativeTrainingData<Scalar> >			Ptr;
	//typedef	boost::shared_ptr<const DerivativeTrainingData<Scalar> >	ConstPtr;
	...
	{
		assert(m_pXd);
		assert(m_pX ? m_pXd->cols() == m_pX->cols() : true);
		return m_pXd->cols();
	}
	 ...
	 * @return	An NdxN matrix const pointer.
	 */
	const MatrixConstPtr sqDistXdX()
	{
		assert(m_pXd && m_pX);
		...
		if(!m_fSqDistXdX)
		{
			m_pSqDistXdX = PairwiseOp<Scalar>::sqDist(m_pXd, m_pX);
			m_fSqDistXdX = true;
		}
	 ...
	 * @return	An NdxNd matrix const pointer.
	 */
	const MatrixConstPtr deltaXd(const int coord)
	{
		assert(m_pXd);
		...
		if(!m_fDeltaListXd)
		{
			...
			for(int d = 0; d < D(); d++)
				m_pDeltaListXd[d] = PairwiseOp<Scalar>::sqDelta(m_pXd, d);
			m_fDeltaListXd = true;
		}
	 ...
	 * @param	[in] coord	Corresponding coordinate. [result]_ij = Xdi_coord - Xj_coord
	 * @return	An NdxN matrix const pointer.
	 */
	const MatrixConstPtr deltaXdX(const int coord)
	{
		assert(m_pXd && m_pX);
		assert(coord >= 0 && coord < D());
		...
		if(!m_fDeltaListXdX)
		{
			m_pDeltaListXdX.resize(D());
			for(int d = 0; d < D(); d++)
				m_pDeltaListXdX[d] = PairwiseOp<Scalar>::sqDelta(m_pXd, m_pX, d);
			m_fDeltaListXdX = true;
		}

		return m_pDeltaListXdX[coord];
	}
	 ...
	 */
	const MatrixPtr sqDistXd(const MatrixConstPtr pXs) const
	{
		assert(m_pXd && pXs);
		return PairwiseOp<Scalar>::sqDist(m_pXd, pXs); // NdxM
	}
	 ...
	 * @return	An NdxM matrix const pointer.
	 */
	const MatrixPtr sqDeltaXd(const MatrixConstPtr pXs, const int coord) const
	{
		assert(m_pXd && pXs);
		return PairwiseOp<Scalar>::sqDelta(m_pXd, pXs, coord); // NdxM
	}
...
protected:
	...
	std::vector<MatrixConstPtr> m_pDeltaXdXList;	// NdxN matrix per each dimension


	/** @brief	Flag for the pre-calculated self squared distances between the derivative training inputs. */
	bool m_fSqDistXd;

	/** @brief	Flag for the pre-calculated cross quared distances between the derivative training inputs and function training inputs. */
	bool m_fSqDistXdX;

	/** @brief	Flag for the pre-calculated self differences between the derivative training inputs. */
	bool m_fDeltaListXd;

	/** @brief	Flag for the pre-calculated cross differences between the derivative training inputs and training inputs. */
	bool m_fDeltaListXdX;
};
