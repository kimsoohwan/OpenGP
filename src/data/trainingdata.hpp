...
public:
	 ...
	 */
	const MatrixPtr sqDist(const MatrixConstPtr pXs) const
	{
		assert(m_pX && pXs);
		return PairwiseOp<Scalar>::sqDist(m_pX, pXs); // NxM
	}
	 ...
	 * @return	An NxM matrix const pointer.
	 */
	const MatrixPtr sqDelta(const MatrixConstPtr pXs, const int coord) const
	{
		assert(m_pX && pXs);
		return PairwiseOp<Scalar>::sqDelta(m_pX, pXs, coord); // NxM
	}
